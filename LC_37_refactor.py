...
class LC_37():
    ...
    # Finding the grid number you are currently in:
    i = 8
    j = 0
    grid_num = 3 * (i // 3) + (j // 3) + 1
    logger.warning(grid_num)

    # TODO: Currently, we have functions to generate rows, columns, and grids, as well as the missing values \
    # TODO: cont.. of each element of those 3. NOW we need to check each cell in the board to see what values
    # TODO: cont.. it can/can't handle. Up until now we have not solved for any cells on the board

    def evaluate_board(board, columns_as_rows, grids_as_rows, missing_from_rows, missing_from_columns, missing_from_grids):

        # TODO: Check board[0][0] to say can the first value from find_missing_digits "rows"
        # to do this, we have to ensure that the first column and the first grid can also accept that value
        # This only gives us possible values and does not directly solve any cells
        ...
        flag_missing_from_column = False
        flag_missing_from_grid = False
        ...
        for x in range(9):
            ...
            for y in range(len(missing_from_rows[i])):
                ...
                if board[i][j] == ".":
                    ...
                    if testing_digit in missing_from_columns[j]:
                        flag_missing_from_column = True
                    else:
                        # can move to the next value because that means the testing_digit lives in the column, so don't have to check the grid
                        if j == 8:
                            j = 0
                            i += 1
                        else:
                            j += 1
                    if testing_digit in missing_from_grids[grid_num-1]:
                        flag_missing_from_grid = True

                    if flag_missing_from_column and flag_missing_from_grid:
                        # this means testing_digit could possibly live in the current cell
                        # TODO: change this logic, but for now....
                        logger.info("Eligible digit")
    ...
    evaluate_board(board, columns_as_rows, grids_as_rows, missing_from_rows, missing_from_columns, missing_from_grids)
